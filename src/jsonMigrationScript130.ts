...
for (const fileName of fileList) {
  ...
  const addresses = {
    ...
    zksync: json.networkAddresses[321],
  }
  ...
  for (const entry of Object.entries(json.networkAddresses)) {
    ...
    const chainData = (await fetch(chainListUrl).then((res) => res.json())) as { rpc: string[] }
    console.log({ chainData })

    const rpc = chainData.rpc.find(
    ...
    if (networkAddresses.length === 0) {
      console.log(`Contract ${fileName} not deployed on chain ${chainId}. Skipping...`)
      continue
  ...
  fs.writeFileSync(`./src/assets/new/v1.3.0/${fileName}`, JSON.stringify(newJson, null, 2))
}
