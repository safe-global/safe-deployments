...
// In v1.3.0, the "canonical" deployer (Arachnid's Deterministic Deployment Proxy) uses a pre-EIP-155 transaction that cannot be
// replayed on EIP-155-enforcing chains, so a separate "eip155" deployment type (via Safe Singleton Factory) was introduced.
// From v1.4.1 onwards, "canonical" itself uses the Safe Singleton Factory, removing the need for a separate "eip155" type.
