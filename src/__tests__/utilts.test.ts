...
describe('utils.ts', () => {
  describe('findDeployment', () => {
    it('should filter by released by default', () => {
      ...
      expect(findDeployment(undefined, testDeployments)).toBe(testReleasedDeployment);

      // should preserve the released flag even if its not explicitly passed
      expect(findDeployment({ network: '1' }, testDeployments)).toBe(testReleasedDeployment);
    });
    it('should return the correct deployment (filtered by version)', () => {
      // Chronological deployments
      expect(findDeployment({ version: '1.3.0' }, _safeDeployments)).toBe(GnosisSafe130);
      expect(findDeployment({ version: '1.2.0' }, _safeDeployments)).toBe(GnosisSafe120);
      expect(findDeployment({ version: '1.1.1' }, _safeDeployments)).toBe(GnosisSafe111);
      expect(findDeployment({ version: '1.0.0' }, _safeDeployments)).toBe(GnosisSafe100);
      // Incorrect filter:
      expect(findDeployment({ version: '2.0.0' }, _safeDeployments)).toBeUndefined();

      // L2 deployments
      expect(findDeployment({ version: '1.3.0+L2' }, _safeL2Deployments)).toBe(GnosisSafeL2130);
      // Incorrect filter:
